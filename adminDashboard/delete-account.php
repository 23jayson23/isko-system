...
<html lang="en">
  <head>
    ...
    <link rel="stylesheet" href="deleted-account.scss" />
    <link
      rel="stylesheet"
                  ...
                  <?php
                        if (isset($_GET['page_no']) && $_GET['page_no']!="") {
                            $page_no = $_GET['page_no'];
                                ...
                                $result_count = $conn->query("SELECT COUNT(*) As total_records FROM `scholar` LEFT JOIN program ON scholar.program_id = program.program_id LEFT JOIN scholar_reqs ON scholar.user_id = scholar_reqs.user_id WHERE scholar.status = 'X' AND scholar_reqs.user_id IS NOT NULL");
                                $total_records = $result_count->fetch_assoc()['total_records'];
                                $total_no_of_pages = ceil($total_records / $total_records_per_page);
                                $second_last = $total_no_of_pages - 1; // total page minus 1
                                $result = $conn->query("SELECT * FROM `scholar` LEFT JOIN program ON scholar.program_id = program.program_id LEFT JOIN scholar_reqs ON scholar.user_id = scholar_reqs.user_id WHERE scholar.status = 'X' AND scholar_reqs.user_id IS NOT NULL ORDER BY scholar.update_date DESC LIMIT $offset, $total_records_per_page");
  ...
  <script>
    ...
    $(document).ready(function(){
      $('.exportDeleted').click(function(){
        window.location.href = '../actions/exportScholar_Deleted.php';
      });
      ...
      $("#search").on("input", function() {
          ...
          if (searchText === "") {
            ...
            $.post('../actions/search_scholar_deleted.php', { key: searchText },
